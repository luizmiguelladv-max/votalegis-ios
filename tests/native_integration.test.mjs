...
test('workflow inclui a integração nativa antes do build', () => {
  ...
  assert.match(fastfile, /PROJECT_ROOT = File\.expand_path\("\.\.", __dir__\)/)
  assert.match(fastfile, /ipa_path = File\.join\(PROJECT_ROOT, "build", "App\.ipa"\)/)
  assert.match(fastfile, /source_script = File\.join\(PROJECT_ROOT, "native", "votalegis-ios\.js"\)/)
